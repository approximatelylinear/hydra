"""Projection head generator: direct FiLM adaptation (no shared matrices)."""
...
class ProjectionHeadGenerator(nn.Module):
    ...
    Applies FiLM (Feature-wise Linear Modulation) directly to base embeddings:
        modulated = gamma * x + beta
        output = normalize(x + alpha * modulated)

    No shared projection matrices — FiLM modulates the base features directly.
    This avoids the collapse caused by trainable shared matrices drifting during
    extended training, while keeping the model simple.
    """
    ...
    def __init__(
        ...
        rank: int = 128,  # unused, kept for API compatibility
    ):
        super().__init__()
    ...
    def apply_head(embeddings: torch.Tensor, head_params: dict[str, torch.Tensor]) -> torch.Tensor:
        """Apply FiLM modulation directly to base embeddings.
        ...
        # Direct FiLM: gamma * x + beta
        modulated = gamma * embeddings + beta
